...
out of the running simulator (``window.CircaTour.captions``), so the words a
viewer hears and the words burned into the screen cannot drift apart.
...
durations; ``record.py`` injects them as ``window.__CIRCA_TIMING`` so the visuals
pace themselves to the voice.
    ...
    python demo_video/record.py          #             record + mux -> circa-demo.mp4
...
SPEECH = HERE / "speech"
URL = sys.argv[1] if len(sys.argv) > 1 else "http://localhost:5173/?tour=1&manual=1"
...
def captions_from_product() -> tuple[list[str], list[str]]:
    """(spoken, shown): the narration, and the on-screen captions for the subtitles.

    They differ only where writing and pronunciation part ways, such as a clock
    time written ``09:02`` and read ``nine oh two``.
    """
    with sync_playwright() as p:
        browser = p.chromium.launch(headless=True)
        page = browser.new_page()
        page.goto(URL, wait_until="load", timeout=60_000)
        page.wait_for_function("() => window.CircaTour?.captions?.length", timeout=60_000)
        captions = page.evaluate("() => window.CircaTour.captions")
        subtitles = page.evaluate("() => window.CircaTour.subtitles")
        browser.close()
...
def main() -> None:
    captions, subtitles = captions_from_product()
    ...
    (HERE / "timing.json").write_text(
        json.dumps({"voice": VOICE, "rate": RATE, "durations": durations,
                    "totalMs": total, "captions": captions, "subtitles": subtitles}, indent=2),
        encoding="utf-8",
    )
    write_subtitles(subtitles, durations)
    print(f"\nnarration.wav  {total / 1000:.1f}s total  ({total / 60000:.2f} min)")
    if total > 175_000:
        print("WARNING: over 2:55. Trim a caption in apps/simulator/src/Tour.tsx.")
...
def write_subtitles(captions: list[str], durations: list[int]) -> None:
    ...
    for i, (caption, ms) in enumerate(zip(captions, durations), start=1):
        ...
        at += ms
    (HERE / "circa-demo.srt").write_text("\n".join(lines), encoding="utf-8")
